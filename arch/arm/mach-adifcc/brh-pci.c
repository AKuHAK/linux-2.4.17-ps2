...
brh_pci_init(void *sysdata)
{
	...
	*BRH_PCI_IO_ATU   = (unsigned int)0x0a000004;
	...
	/*
	 *
	 * We setup the top inbound window to be from 0xc0000000 to
	 * 0xc2000000.  We can't use both 32MB windows because a
	 * transaction that crosses the boundary of the two windows
	 * wraps around to the beggining of the first window instead
	 * of being forwarded to the next window. :(
	 */
	if(!system_rev) {
		...
		*BRH_PCI_MEM0_XLATE = 0x00000001;
		...
		/*
		 * We disable the 2nd window by targetting it at address 0
		 */
		brh_write_config_dword(fake_pci_dev(hose, 0, 0),
		...
		/*
		 * Disable the two 32M windows and enable the single
		 * 128MB window.
		 */
		brh_write_config_dword(fake_pci_dev(hose, 0, 0), 
