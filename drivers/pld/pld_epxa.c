...
int pld_ioctl(struct inode *inode, struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case PLD_IOC_ADD_PLD_DEV:
	{
		...
		if(result)
			return -EFAULT;
		/* Remember that the name string needs null termination! */
		name=kmalloc(info.nsize+1,GFP_KERNEL);
		if(!name)
			return -ENOMEM;
		...
		/* 
		 * Check for a global data block. Currently this only describes
		 * the irq controller mode and the kernel only supports 
		 * irq mode 3. So if the data says different issue a warning.
		 */
		if(!info.vendor_id&&!info.product_id){	
			printk(KERN_INFO "IRQ mode set to %d\n,",*(char*)data);
			if(*(char*)data!=3){
				printk(KERN_WARNING "This kernel only supports IRQ mode 3\n");
				printk(KERN_WARNING "IRQ mode set to 3\n");
			}
			goto ioctl_out1;
		}
	...
	case PLD_IOC_SET_INT_MODE:
		if(arg==3){
			...
			printk(KERN_INFO "There is no interrupt handler available for this mode (%d).\
